...
impl<'a> OpenTagBuilder<'a> {
    ...
    fn add_attr_name(&mut self, name: Cow<'a, str>) {
    ...
    fn build(self, self_closing: bool) -> OpenTag<'a> {
        OpenTag {
            name: self.name,
...
pub struct Tokenizer<'a> {
    ...
    tokens: Vec<Token<'a>>,
    open_tag_builder: Option<OpenTagBuilder<'a>>,
    comment: String,
}
...
impl<'a> Tokenizer<'a> {
    ...
    fn handle_tag_attr(&mut self, ch: char) -> TokenizeResult<'a, ()> {
        if ch == '>' || ch == '=' || ch.is_whitespace() {
            ...
            self.open_tag_builder
                .as_mut()
                .unwrap()
                .add_attr_name(tag_attr_name.into());
            self.state = if ch == '>' {
                self.finalize_open_tag(false);
                TokenizerState::Text
            } else if ch == '=' {
    ...
    fn handle_tag_name(&mut self, ch: char) -> TokenizeResult<'a, ()> {
        if ch == '>' || ch.is_whitespace() {
            ...
            if self.is_end_tag {
                self.tokens.push(Token::CloseTag(tag.into()));
            } else {
                if token_finalized {
                    ...
                    self.open_tag_builder = Some(OpenTagBuilder::new(tag.into()));
                }
            }
        ...
        assert_eq!(
            ...
            Ok(vec![
                Token::OpenTag(OpenTag {
                    ...
                    tag_attrs: vec![
                        TagAttr {
                            name: "attr1".to_owned().into(),
                            value: None
                        },
                        TagAttr {
                            name: "attr2".to_owned().into(),
                            value: None
                        },
                    ],
                    self_closing: false,
...
<custom attr1></custom>
        ...
        assert_eq!(
            ...
            Ok(vec![
                ...
                new_line_text(),
                Token::OpenTag(OpenTag {
                    name: "html".to_owned().into(),
                    tag_attrs: vec![],
                    self_closing: false
                }),
                new_line_text(),
                Token::OpenTag(OpenTag {
                    name: "head".to_owned().into(),
                    tag_attrs: vec![],
                    self_closing: false
                }),
                new_line_text(),
                Token::OpenTag(OpenTag {
                    name: "title".to_owned().into(),
                    tag_attrs: vec![],
                    self_closing: false
                }),
                ...
                new_line_text(),
                Token::OpenTag(OpenTag {
                    name: "img".to_owned().into(),
                    tag_attrs: vec![TagAttr {
                        name: "src".to_owned().into(),
